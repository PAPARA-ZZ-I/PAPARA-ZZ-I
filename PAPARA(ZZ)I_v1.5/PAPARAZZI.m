...
cversion = '1.5'; % current version
cdate = 'November 2015'; % current date
...
nmax = length(imagelist); % max number of images
...
cbPop1 = horzcat('tmpsel = get(hlist,''Value''); ',...
    ...
    'fcn_popup1(hpop1,hbutton1,hlist,objWoRMS,kwlist,WoRMSlist,WoRMSrecords,first_match); ',...
...
cbExport = horzcat('switch get(hPoints,''State''), ',...
    ...
    'end;');
cbHelp = horzcat('if exist(helpfile,''file'')==2, open(helpfile); ',...
