...
sumpath = [inpath 'summary' sep];
if isdir(sumpath)==0, mkdir(sumpath); end
switch annotype
    case 'Annotation'
        sumfile1 = [sumpath userid '_summary_Annotations_WholeImage.txt'];
        sumfile2 = [sumpath userid '_summary_Annotations_UsableArea.txt'];
        anglefile1 = [sumpath userid '_summary_Angles_WholeImage.txt'];
        anglefile2 = [sumpath userid '_summary_Angles_UsableArea.txt'];
    ...
    case 'GeneratedPoint'
        sumfile1 = sprintf('%s%s_summary_%li%s%sPoints_WholeImage.txt',...
            sumpath,userid,pointmax,upper(pointtype(1)),pointtype(2:end));
        sumfile2 = sprintf('%s%s_summary_%li%s%sPoints_UsableArea.txt',...
            sumpath,userid,pointmax,upper(pointtype(1)),pointtype(2:end));
        anglefile1 = sprintf('%s%s_summary_Angles_%li%s%sPoints_WholeImage.txt',...
            sumpath,userid,pointmax,upper(pointtype(1)),pointtype(2:end));
        anglefile2 = sprintf('%s%s_summary_Angles_%li%s%sPoints_UsableArea.txt',...
            sumpath,userid,pointmax,upper(pointtype(1)),pointtype(2:end));
...
fidout1 = fopen(sumfile1,'w');
fidout2 = fopen(sumfile2,'w');
...
for k1=kmax:-1:1 % go backward!!!
    ...
    while ~feof(fid)
        tline = fgetl(fid);
        if tline ~= -1
            strline = textscan(tline,'%f %f %s %f %f %f %f','delimiter','\t');
            x = strline{1};
            y = strline{2};
            kw = strline{3}{1};
            if ~isempty(strline{4}) && ~isempty(strline{5}) ...
                    && ~isempty(strline{6}) && ~isempty(strline{7})
                sz = sqrt( (strline{4}-strline{6})^2 + (strline{5}-strline{7})^2 ); % length of feature in pixel
                szvec = [strline{4}-strline{6} ; -(strline{5}-strline{7})]; % end point of the vector minus start point
            else
                sz = [];
                szvec = [];
            end
            ...
            for k2=1:numel(kwlist)
                if strcmp(kw,kwlist{k2})==1
                    ...
                    if ~isempty(sz) && scm~=0 && scpxl~=0 % Add size measurement to list
                        
                        angvec(k1,k2,1) = angvec(k1,k2,1) + szvec(1); % sum of x vectors per image
                        angvec(k1,k2,2) = angvec(k1,k2,2) + szvec(2); % sum of y vectors per image
                        
                        pos = featszCnt(1,k2) + 1;
                        if pos > size(featsz,1) % add a row to the variable
                            featsz = [featsz ; zeros(1,size(featsz,2))];
                        end
                        featsz(pos,k2) = scm * sz/scpxl; % size is converted in metres
                        featszCnt(1,k2) = pos;
                    end
                    ...
                    if exist('rectX','var')==1 && exist('rectY','var')==1 ...
                        ...
                        if ~isempty(sz) && scm~=0 && scpxl~=0 % Add size measurement to list
                            ...
                            angvecS(k1,k2,1) = angvecS(k1,k2,1) + szvec(1); % sum of x vectors per image
                            angvecS(k1,k2,2) = angvecS(k1,k2,2) + szvec(2); % sum of y vectors per image
                            
                            pos = featszCntS(1,k2) + 1;
                            if pos > size(featszS,1) % add a row to the variable
                                featszS = [featszS ; zeros(1,size(featszS,2))];
                            end
                            featszS(pos,k2) = scm * sz/scpxl; % size is converted in metres
                            featszCntS(1,k2) = pos;
                        end
                    end
...
fclose(fidout4);
delete(hwb);
msg = sprintf('%s\n%s','Export successful. The summary files are in:',sumpath);
