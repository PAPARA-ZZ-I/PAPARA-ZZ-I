...
n = 1;
sc = 1;
dr = 1; % display resolution [mm per screen pxl]
drflag = 0; % activate/deactivate display resolution limit
...
cversion = '2.3'; % current version
cdate = 'November 2016'; % current date
...
cbMeas = horzcat('hToolbar = findall(gcf,''Tag'',''FigureToolBar''); ',...
    'if ~isempty(findobj(gca,''Selected'',''on'')) && ~numel(aflag), ',...
    ...
    'set(findobj(gca,''Tag'',annotype),''HitTest'',''on''); end; ',...
...
cbSaveDefault = horzcat('if exist(''imformat'',''var'')~=1, ',...
    ...
    'case ''off'', fcn_save_image(n,imagelist,inpath,userid,sep,gca,imformat,annotype); ',...
    'case ''on'', fcn_save_image(n,imagelist,inpath,userid,sep,gca,imformat,annotype,pointtype,pointmax); ',...
    'end;',...
    'case {''EPS2'',''EPS3''},', ...
    'switch get(hPoints,''State''), ',...
    'case ''off'', fcn_save_eps(n,imagelist,inpath,userid,sep,gca,imformat,annotype); ',...
    'case ''on'', fcn_save_eps(n,imagelist,inpath,userid,sep,gca,imformat,annotype,pointtype,pointmax); ',...
...
cbDr = horzcat('drflag = get(h_drcb,''Value''); ',...
    'fcn_displayresolution(h,h_dr,drflag,[],n,imagelist,inpath,userid,sep);');
cbDispRes = horzcat('if isnan(str2double(get(h_dr,''String'')))',...
    '|| str2double(get(h_dr,''String'')) <= 0 ',...
    'set(h_dr,''String'',dr); ',...
    'else dr = str2double(get(h_dr,''String'')); end; ',cbDr);
...
[hPoints,hSave] = fcn_ctrl_FigTools(fig_main,'off',cbPrev,cbNext,cbMeas,cbRect,...
    cbIgnore,cbPoints,cbSave,cbExport,cbHelp,cbAbout);
...
uicontrol('Parent',gp_left,'Style','text',...
    'String','scale [m]:',...
    'HorizontalAlignment','right','Units','normalized',...
    'Position',[.1 .080 .24 .035]);

h_sc = uicontrol('Parent',gp_left,'Style','edit',...
    'BackgroundColor',[1 1 1],'String',sc,...
    'Units','normalized','Position',[.38 .080 .24 .035],...
    'CallBack',cbScaleVal);

uicontrol('Parent',gp_left,'Style','pushbutton',...
    'BackgroundColor',ButtonColor,...
    'String','Apply','TooltipString','Apply scale value',...
    'Units','normalized','Position',[.65 .080 .24 .035],...
    'CallBack','fcn_applyscale(n,imagelist,inpath,aflag,sc,userid,sep);');

uicontrol('Parent',gp_left,'Style','text',...
    'String','Max. zoom [mm/screen pxl]:',...
    'HorizontalAlignment','right','Units','normalized',...
    'Position',[.1 .045 .24 .035]);

h_dr = uicontrol('Parent',gp_left,'Style','edit',...
    'BackgroundColor',[1 1 1],'String',dr,...
    'Units','normalized','Position',[.38 .045 .24 .035],...
    'CallBack',cbDispRes);

h_drcb = uicontrol('Parent',gp_left,'Style','checkbox',...
    'BackgroundColor',ButtonColor,...
    'String','Enable','TooltipString','Enable/disable display resolution control',...
    'Units','normalized','Position',[.65 .045 .24 .035],...
    'CallBack',cbDr);
...
%% other
[him,fid,sc,annotype,rectfile] = fcn_disp_image(n,imagelist,inpath,h,h_sc,h_dr,drflag,0,userid,sep,kwlist,filterlist,infotxt);
[selec,fid,~] = list_callback(fid,hlist,him,infotxt,annotype);
