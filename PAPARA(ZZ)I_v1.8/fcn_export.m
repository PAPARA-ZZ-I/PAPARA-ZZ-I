...
switch annotype
    case 'Annotation'
        sumfile1 = [sumpath userid '_' strdate '_Summary_FreeAnnotations_WholeImage.txt'];
        sumfile2 = [sumpath userid '_' strdate '_Summary_FreeAnnotations_UsableArea.txt'];
        anglefile1 = [sumpath userid '_' strdate '_Angles_FreeAnnotations_WholeImage.txt'];
        anglefile2 = [sumpath userid '_' strdate '_Angles_FreeAnnotations_UsableArea.txt'];
    ...
    case 'GeneratedPoint'
        sumfile1 = sprintf('%s%s_%s_Summary_%li%s%sPoints_WholeImage.txt',...
            sumpath,userid,strdate,pointmax,upper(pointtype(1)),pointtype(2:end));
        sumfile2 = sprintf('%s%s_%s_Summary_%li%s%sPoints_UsableArea.txt',...
            sumpath,userid,strdate,pointmax,upper(pointtype(1)),pointtype(2:end));
        anglefile1 = sprintf('%s%s_%s_Angles_%li%s%sPoints_WholeImage.txt',...
            sumpath,userid,strdate,pointmax,upper(pointtype(1)),pointtype(2:end));
        anglefile2 = sprintf('%s%s_%s_Angles_%li%s%sPoints_UsableArea.txt',...
            sumpath,userid,strdate,pointmax,upper(pointtype(1)),pointtype(2:end));
    ...
    while ~feof(fid)
        ...
        if tline ~= -1
            ...
            if ~isempty(strline{4}) && ~isempty(strline{5}) ...
                ...
                sz = sqrt( (pt1(1)-pt2(1))^2 + (pt1(2)-pt2(2))^2 );
            ...
            for k2=1:numel(kwlist)
                if strcmp(kw,kwlist{k2})==1
                    ...
                    if ~isempty(sz) && scm~=0 && scpxl~=0 % Add size measurement to list
                        ...
                        angvec(k1,k2,1) = angvec(k1,k2,1) + szvec(1)/sz; % sum of normalized x vectors per image
                        angvec(k1,k2,2) = angvec(k1,k2,2) + szvec(2)/sz; % sum of normalized y vectors per image
                    ...
                    if exist('rectX','var')==1 && exist('rectY','var')==1 ...
                        ...
                        if ~isempty(sz) && scm~=0 && scpxl~=0 % Add size measurement to list
                            ...
                            angvecS(k1,k2,1) = angvecS(k1,k2,1) + szvec(1)/sz; % sum of normalized x vectors per image
                            angvecS(k1,k2,2) = angvecS(k1,k2,2) + szvec(2)/sz; % sum of normalized y vectors per image
