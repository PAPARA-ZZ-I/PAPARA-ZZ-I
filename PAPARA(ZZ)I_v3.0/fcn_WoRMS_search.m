...
if isstring(searchstr), searchstr = strtrim(searchstr); end % remove leading and trailing spaces
...
if ~isempty(recordstruct)
    ...
    for k = 1:numel(recordstruct)
        ...
        sNameAphiaID = [recordstruct(k).scientificname,' [',...
            sprintf('%i',recordstruct(k).AphiaID),']'];
...
% Concatenate previous results if they exist
