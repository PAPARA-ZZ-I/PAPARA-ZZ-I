...
if cPointPos(1,1) >= imleft && cPointPos(1,1) <= imright ...
    ...
    zoomfactor = max(0.1,zoomfactor); % prevent errors when zoomfactor<=0
    ...
    % limit 'zoom in' when new change is less than 1 pixel wide
    if zoomfactor < 1 && (newDeltaX < 1 || newDeltaY < 1)
        return;
    end
