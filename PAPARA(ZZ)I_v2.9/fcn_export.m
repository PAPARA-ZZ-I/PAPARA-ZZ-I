        ...
        if tline ~= -1
            ...
            for k2=1:numel(kwlist)
                if strcmp(kw,kwlist{k2})==1
                    ...
                    if exist('uaX','var')==1 && exist('uaY','var')==1 ...
                            && ~isempty(uaX) && ~isempty(uaY) ...
                            && inpolygon(x, y, uaX, uaY)
...
for k1=1:size(counts,1)
    ...
    if ua_area == 0 % if no usable_area, use the whole image counts
        ...
        tline2 = sprintf('%s\t%s\t%f',tline,'selected area',ua_area);
