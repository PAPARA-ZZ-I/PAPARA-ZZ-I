function [him,fid,sc,annotype,uafile,polyfile] = fcn_disp_image(n,imagelist,inpath,...
    h,hhideshow,h_sc,h_ds,h_ppi,dsflag,aflag,userid,sep,kwlist,filterlist,...
    infotxt,pointtype,pointmax)
...
polypath = [inpath userid '_polygon' sep];
...
if fid ~= -1
    ...
    CStr = Data{1};
    fclose(fid);
    fid = -1; % for some reason, the previous line does not reset fid.
    
    idC = strfind(CStr,imagelist{n});
    id = find(~cellfun('isempty',idC), 1);
    
    if exist('id','var')==1 && ~isempty(id) % if image is in the list of images to ignore
...
uafile = [ uapath iname '.txt' ];
polyfile = [ polypath iname '.txt' ];
...
switch onoff
    case 'on' % display generated points
        ...
        him = fcn_plot_annotations(h,txtfile,n,imagelist,inpath,...
            uafile,scfile,aflag,kwlist,filterlist,infotxt,annotype,...
            hhideshow,h_ds,h_ppi,dsflag);
