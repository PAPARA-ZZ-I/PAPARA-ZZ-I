...
switch get(hPoints,'State')
    case 'off'
        [him,fid,sc,annotype,rectfile] = fcn_disp_image(n,imagelist,inpath,h,hhideshow,h_sc,h_ds,h_ppi,dsflag,aflag,...
            userid,sep,kwlist,filterlist,infotxt);
    case 'on'
        [him,fid,sc,annotype,rectfile] = fcn_disp_image(n,imagelist,inpath,h,hhideshow,h_sc,h_ds,h_ppi,dsflag,aflag,...
            userid,sep,kwlist,filterlist,infotxt,pointtype,pointmax);
end
imCData = get(him,'CData');
imdbl = fcn_im2dbl(imCData); % convert to double precision for image enhancements
hSliders = [hbrightness;hcontrast;hgamma];
set(hSliders,'Value',0); % reset sliders
