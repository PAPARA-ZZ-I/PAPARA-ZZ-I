...
n = 1;
sc = 1;
ds = 1; % display size ['image cm' per 'display cm']
...
cbDispRes = horzcat('if isnan(str2double(get(h_ds,''String'')))',...
    '|| str2double(get(h_ds,''String'')) <= 0 ',...
    'set(h_ds,''String'',ds); ',...
    'else ds = str2double(get(h_ds,''String'')); end; ',cbDs);
...
uicontrol('Parent',gp_left,'Style','text','FontUnits','normalized',...
    'FontSize',.4,'String','Max. zoom [cm/cm]:',...
    'TooltipString','Maximum display scale [scene cm / display cm]',...
    'HorizontalAlignment','right','Units','normalized',...
...
h_ds = uicontrol('Parent',gp_left,'Style','edit',...
    ...
    'TooltipString',['Maximum display scale in ''scene centimetres'' per ',...
    '''display centimetre'' [scene cm / display cm].'],'CallBack',cbDispRes);
...
h_dscb = uicontrol('Parent',gp_left,'Style','checkbox',...
    ...
    'String','Enable','TooltipString','Enable/disable display size control',...
...
%% other
[him,fid,sc,annotype,rectfile] = fcn_disp_image(n,imagelist,inpath,h,h_sc,h_ds,h_ppi,dsflag,0,userid,sep,kwlist,filterlist,infotxt);
[selec,fid,~] = list_callback(fid,hlist,him,infotxt,annotype);
